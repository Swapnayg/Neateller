...
class AdminUsers_Booking(ImportExportModelAdmin,admin.ModelAdmin):
    list_display = ['_service_name','_booking_id','_confirm_id','_payment_mode','_booking_status','_invoice_url','created_at','updated_at','edit_invoice_mode']
...
def get_admin_urls(urls):
    def get_urls():
        my_urls =  [
           ...
           path('invoice_edit/<str:url>/<str:confirmid>/', invoiceEdit_View, name='invoice_edit' ),
        ]
        return my_urls + urls
    return get_urls
...
def invoiceEdit_View(request,url='',confirmid = ''):
    return render(request , 'InvoiceEdit.html',{'invoiceurl':url,'confirmId':confirmid})
