...
class quote_View(View):
    ...
    def get(self , request,Id=0):
        img = ''
        if(Id == "electricians"):
            img = '/static/assets/uploads/media-uploader/electricians.jpg'
        elif(Id == "plumbers"):
            img = '/static/assets/uploads/media-uploader/plumbers.jpg'
        elif(Id == "masons"):
            img = '/static/assets/uploads/media-uploader/mason.jpg'    
        return render(request , 'quote.html', {'serviceName': Id,'serviceImage':img,'services':  ServiceCategories.objects.all()})
...
class text_View(View):
    return_url = None
    def get(self , request,Id=0):
        return render(request , 'text.html', {'services':  ServiceCategories.objects.all()})
...
class booking_View(View):
    ...
    def get(self , request,Id=0):
        ...
        return render(request , 'booking1.html' , {'page':serviced.name,'image':serviced.image,'services':  ServiceCategories.objects.all()})
...
class refund_View(View):
    return_url = None
    def get(self , request,Id=0):
        return render(request , 'refund.html',{'services':  ServiceCategories.objects.all()})
...
def EmailSendwithAttachmentFunction(sendmessage,toaddress,subject,cusName,invoiceName):
    ...
    url = "https://skillyservices.com/media/invoice/invoice_CHK-10351720.pdf"
...
<body marginheight="0" topmargin="0" marginwidth="0" style="margin: 0px; background-color: #f2f3f8;" leftmargin="0">
    ...
    <span style="display:inline-block; vertical-align:middle; Padding:15px;padding-top: 0px;">Thank you for making a """+serviceName+""" service booking with Skilly Services! You will receive another email shortly confirming your booking.</span>
	  <span style="display:inline-block; vertical-align:middle; Padding:15px;padding-top: 0px;"> In the meantime, please go over your booking details under your profile booking section and let us know if you would like to make any changes by replying to this email.</span>
  </td> 
                                </tr>
                                <tr style="text-align:left;">
        ...
        mail_content_admin = ''
        error_message = EmailSendFunction(mail_content,emailAddress,"We have received your booking",compleName)
        error_message = EmailSendFunction(mail_content,'support@skillyservices.com',"We have received an new booking",compleName)
...
@csrf_exempt
def invoice_savePdf_View(request):
    if request.method == 'POST':
        files = request.FILES.getlist("file")
        uconfirmId = request.POST.get("confirmId")
        if len(files) != 0:
            for file in files:
                bookingDetails = User_Booking_Details.objects.get(_confirm_id = uconfirmId )
                invoiceName = str(bookingDetails._booking_id).replace("-r1","")
                os.remove('D:/Python/Neateller/media/invoice/invoice_'+invoiceName+'.pdf')
                fs= FileSystemStorage(location= settings.MEDIA_ROOT +'/invoice/')
                file_path=fs.save('invoice_'+invoiceName+'.pdf',file) 
                bookingDetails._invoice_url = '/invoice/'+ file_path
                bookingDetails.save()
            else:
                print('No File')
        return HttpResponse('sucess')
...
@csrf_exempt
def invoice_sendPdfView(request):
    if request.method == 'POST':
        uconfirmId = request.POST.get("u_confirmid")
        bookingDetails = User_Booking_Details.objects.get(_confirm_id = uconfirmId )
        userDetails = User.objects.get(email=bookingDetails._userid)
        u_serv_Date = bookingDetails._service_date
        service_Time = bookingDetails._service_time
        u_cusName = userDetails.username
        u_serv_Name = bookingDetails._service_name
        message = """
<!doctype html>
<html lang="en-US">
...
<head>
    <meta content="text/html; charset=utf-8" http-equiv="Content-Type" />
    <title>SkillY - Thank You Skilly Services</title>
    <meta name="description" content="Thank You Skilly Services">
    <style type="text/css">
        a:hover {text-decoration: underline !important;}
    </style>
</head>
                    ...
                    <tr>
                        <td>
                            <table width="95%" border="0" cellpadding="0" cellspacing="0"
                                ...
                                <tr>
                                    <td style="text-align:center;">
                                        <h1 style="color:#e424b7; font-weight:600; margin:0;font-size:28px;font-family:'Rubik',sans-serif;">Happy Customer - Skilly Services.</h1>
                                    </td>
                                </tr>
                                <tr style="text-align:left;">
                                  <td style="">    <span style="display:inline-block; vertical-align:middle; Padding:15px;">Mr./Mrs. """+u_cusName+""",</span>
    <span style="display:inline-block; vertical-align:middle; Padding:15px;padding-top: 0px;text-align:justify;line-height:20px;">Dear , Skilly customer your service for <b>"""+u_serv_Name+"""</b> have been sucessfully completed by our representative on Date : <b>"""+u_serv_Date+"""</b> At : <b>"""+service_Time+"""</b>. and the corresponding invoice to this service is been attached to this email. </span>
	  <span style="display:inline-block; vertical-align:middle; Padding:15px;padding-top: 0px;"> </span>
  </td> 
                                </tr>
                                <tr style="text-align:left;">
                                   <span style="display:inline-block; vertical-align:middle; Padding:15px;">If you have any questions, we're happy to help. Please reach out to us at support@skillyservices.com or +971 55 548 7771.</span>
        ...
        u_subject = "Thank For Visiting Skilly Services."
        invoiceName = str(bookingDetails._invoice_url).split('/')[2]
        emailstatus = EmailSendwithAttachmentFunction(message,u_email,u_subject,u_cusName,invoiceName)
        print(emailstatus)
        return HttpResponse('sucess')